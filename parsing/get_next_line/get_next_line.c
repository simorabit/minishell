...
/*   Updated: 2024/06/10 16:31:52 by mal-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*save_data(char *container)
{
	...
	j = 0;
	i = 0;
	if (!container || !ft_mstrchr(container, '\n'))
		return (NULL);
	len = ft_mstrlen(container);
	while (container[i] != '\n')
		i++;
	len = len - i;
...
static char	*get_line(int fd, char *buf, char **container)
{
	...
	reader = 1;
	if (!*container)
		*container = ft_mstrdup("");
	while (reader && !ft_mstrchr(*container, '\n'))
	{
		reader = read(fd, buf, BUFFER_SIZE);
		if (reader == -1)
			return (NULL);
		buf[reader] = '\0';
		*container = ft_mstrjoin(*container, buf);
	}
	if (reader == 0 && (!*container || !**container))
		return (NULL);
	if (ft_mstrchr(*container, '\n'))
	{
		while ((*container)[i] != '\n')
			i++;
