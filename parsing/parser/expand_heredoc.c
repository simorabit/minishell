...
/*   Updated: 2024/08/01 20:07:52 by mal-mora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../minishell.h"

char	*handel_double_q_here(char *result, char *s, t_int *sin_t, t_env **env_list)
{
	int		k;
	char	*res;
...
char	*handel_double_q_here2(char *result, char *s, t_int *sin_t, t_env **env_list)
{
	int		k;
	char	*res;
...
char	*handel_expand_quotes_here(t_int *is, char *s, char **res, t_env **env)
{
	if (s[is->i] == DOUBLE_QUOTE)
		*res = handel_double_q_here(*res, s, is, env);
	else
		*res = handel_double_q_here2(*res, s, is, env);
	return (*res);
}
